...
using namespace std;
MysqlDataBase::MysqlDataBase(const string& hostname, const string& username, const string& password, const string& schema)
    : hostname(hostname), username(username), password(password), schema(schema) {
...
sql::Connection* MysqlDataBase::establishConnection() {
    sql::Driver *driver;
    try {
        driver = get_driver_instance(); 
        conn = driver->connect("tcp://" + hostname + ":3306", username, password);
...
//call procedures
int MysqlDataBase::callUpdateEmployeeAgeProcedure() {
