...
int main() {
    ...
    while (true) {
        ...
        // Select
        if (select(FD_SETSIZE, &tempReadFds, nullptr, nullptr, nullptr) == -1) {
            perror("Select error");
            exit(EXIT_FAILURE);
        }
        ...
        for (int i = 0; i < MAX_CLIENTS; ++i) {
            if (clientSockets[i] > 0 && FD_ISSET(clientSockets[i], &tempReadFds)) {
                ...
                if (read(clientSockets[i], buffer, BUFFER_SIZE) <=0 ){
                    ...
                    for (int j = 0; j < MAX_CLIENTS; ++j) {
                        if (clientSockets[j] > 0 && clientSockets[j] != clientSockets[i]) {
                            if (send(clientSockets[j], buffer, strlen(buffer), 0) == -1) {
                                perror("Send error");
